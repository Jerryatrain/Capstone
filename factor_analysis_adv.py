...
def get_new_stock_filter(stock_list,date_list, newly_listed_threshold = 252):
    ...
    newly_listed_window = pd.Series(index=stock_list, 
                                    data=[rqdatac.get_next_trading_date(listed_date, n=newly_listed_threshold) for listed_date in listed_date_list]) 
    # 防止分割日在研究日之后，后续填充不存在
...
# 2.3 行业市值中性化
def neutralization(df,order_book_ids, tmp_path, index_item = ''):
    ...
    try:
        # 获取存储数据
        df_industry_market = pd.read_pickle(f'./{tmp_path}/tmp/df_industry_market_{index_item}_{start}_{end}.pkl')
    except:
        # 获取市值暴露度
        ...
        create_dir_not_exist(f'./{tmp_path}/tmp')
        df_industry_market.to_pickle(f'./{tmp_path}/tmp/df_industry_market_{index_item}_{start}_{end}.pkl')
...
# 2.3.1 获取行业暴露度举证
def get_industry_exposure(order_book_ids,datetime_period,industry = 'zx'):
    
    """
    :param order_book_ids: 股票池 -> list
    :param datetime_period: 研究日 -> list
    :param industry: 行业分类标准 二选一 中信/申万 zx/sw -> str
    :return result: 虚拟变量 -> df_unstack
    """

    if industry not in ['zx','sw']:
    ...
    df = df.reindex(index = date_list_base).ffill().reindex(index = datetime_period)[order_book_ids].ffill()
    
    #生成行业虚拟变量
    return pd.get_dummies(df.stack())


# 2.4 因子清洗
def data_clean(df,index_fix, tmp_path, index_item = ''):
    """
    :param df: 因子值 -> df_unstack
    ...
    try:
        combo_mask = pd.read_pickle(f'./{tmp_path}/tmp/combo_mask_{index_item}_{start_date}_{end_date}.pkl')
    except:
        # 新股过滤
        ...
        create_dir_not_exist(f'./{tmp_path}/tmp')
        combo_mask.to_pickle(f'./{tmp_path}/tmp/combo_mask_{index_item}_{start_date}_{end_date}.pkl')
    
    df = df.mask(~combo_mask).dropna(how = 'all',axis = 1)
    # 离群值 标准化 中性化 标准化
    df = standardize(neutralization(standardize(mad(df)),stock_list, tmp_path,index_item))
    df = df.apply(lambda x: x.astype(float))

    try:
        limit_up_filter = pd.read_pickle(f'./{tmp_path}/tmp/limit_up_filter_{index_item}_{start_date}_{end_date}.pkl')
    except:
        # 涨停过滤
        limit_up_filter = get_limit_up_filter(stock_list,date_list)
        limit_up_filter.to_pickle(f'./{tmp_path}/tmp/limit_up_filter_{index_item}_{start_date}_{end_date}.pkl')
...
# 3.1 IC计算  
def Quick_Factor_Return_N_IC(df,n, tmp_path,index_item,name = '',Rank_IC = True):
    ...
    try:
        open = pd.read_pickle(f'./{tmp_path}/tmp/open_{index_item}_{start}_{end}.pkl')
    except:
        ...
        create_dir_not_exist(f'./{tmp_path}/tmp')
        open.to_pickle(f'./{tmp_path}/tmp/open_{index_item}_{start}_{end}.pkl')
